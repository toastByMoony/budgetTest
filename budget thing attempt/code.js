...
let currentButton = document.getElementById('redCurrentButtonStuff');
...
let redCurrentDetailsStuff=document.getElementById('redCurrentDetailsStuff');
let redCurrentDetailsClass=document.getElementsByClassName('redCurrentDetail');
let redCurrentButton=document.getElementById('currentStandingsButton');
let redCurrentWord=document.getElementById('currentStandingsWord');
let redExpendFieldsStuff=document.getElementById('redExpendFieldsStuff');

//Advanced redistribution stuff
let advStartDate=document.getElementById('advStartDate');
let advTermSelect=document.getElementById('advTermSelect');
let advTermEnd=document.getElementById('advTermEnd');
let advGoalDaily=document.getElementById('advGoalDaily');
let advMinDaily=document.getElementById('advMinDaily');
let advDistMethod=document.getElementById('advDistMethod');
//general variables

let saveButton=document.getElementById('saveButton');
let notification = document.getElementById('notification');
let menuShowing;
let notifBackground=document.getElementById('notifBackground');
let hide=()=>notifBackground.style.display='none';
let show=()=>notifBackground.style.display='initial';
hide();
saveButton.addEventListener('click', () => {
    menuShowing?hideSaveMenu():showSaveMenu();
});

let copyCodeButton = document.getElementById('copyCode');
let inputCodeButton = document.getElementById('inputCode');
notification.style.display='none';
copyCodeButton.addEventListener('click', ()=>{
    let textDisplay = document.createElement('textarea');
    textDisplay.id='cookieCodeDisplay';
    textDisplay.value=encodeCookies();
   notification.innerHTML='Copy:';
   notification.append(document.createElement('br'));
   notification.append(textDisplay);
   let a=document.getElementById('cookieCodeDisplay');
   notification.style.display='initial';
   a.select();
   a.setSelectionRange(0,999999);
   show();
   notifBackground.addEventListener('click', ()=>{hide();notification.style.display='none'});
});

inputCodeButton.addEventListener('click', ()=>{
    notification.innerHTML="Paste code!";
    notification.append(document.createElement('br'));
    let input = document.createElement('input');
    input.id='codeInput';
    input.type='text';
    notification.append(input);
    input.addEventListener('keypress', (k)=>{
        k==='Enter'?()=>{decodeCookies(input.value);hide();}:console.log('notEnter');
    })
    notifBackground.addEventListener('click',()=>{console.log(input.value);decodeCookies(input.value);hide()});
    notification.style.display='initial';
    show();
})
...
if(document.title==="Home page"){
    ...
    redCurrentDetailsStuff.style.display='none';
    //current date 
    ...
    redCurrentDetailsStuff.addEventListener('mouseover', showRedDetails);
    redCurrentDetailsStuff.addEventListener('mouseleave', hideRedDetails);
    currentButton.addEventListener('click', setRedAsCurrent);
    } else {
        currentButton.style.display='none';
    }

    redCurrentDetailsStuff.addEventListener('mouseenter',showRedDetails);
    redCurrentDetailsStuff.addEventListener('mouseleave', hideRedDetails);

    redCurrentButton.addEventListener('mouseenter', showRedDetails);
    redCurrentButton.addEventListener('mouseleave', hideRedDetails);

    redCurrentWord.addEventListener('mouseenter',showRedDetails);
    redCurrentWord.addEventListener('mouseleave', hideRedDetails);
    ...
    loadRedSave();
} else if(document.title==='Advanced Redistribution'){
...
function showRedDetails(){
    console.log('hello');
    redCurrentDetailsStuff.style.display='initial';
}
...
function redExpendFields(){
    ...
    redExpendFieldsStuff.innerHTML=null;
    if(numOfExpend===0||numOfExpend===null||numOfExpend===NaN||numOfExpend<=0){
        redExpendFieldsStuff.style.display="none";
        return;
    }
        redExpendFieldsStuff.style.display="flex";
    for(let i=0;i<numOfExpend;i++){
        let redSubImage=document.createElement('img');
        redSubImage.src="./redistribution-stickies/pink-with-flowers-removebg-preview (1).png";

        let rotation=Math.floor(Math.random()*30)-15;

        let redSubContainer = document.createElement('div');
        redSubContainer.className="redSubItem";
        ...
        redSubContainer.style.transform=`rotate(${rotation}deg)`;
        redSubContainer.append(redSubImage,newRedExpendNameField, newRedExpendValueField);
        redExpendFieldsStuff.append(redSubContainer);
    }
}
...
function setAdvEndOption(){
    let advEndTitle=document.createElement('label');
    advEndTitle.setAttribute('for', 'advEndInput');
    let advEndInput=document.createElement('input');
    switch(advTermSelect.value){
        case 'auto':
        let daysRemaining = new Date(advStartDate.value).getTime();
        console.log(daysRemaining);
        console.log(new Date(advStartDate.value));
        daysRemaining -= new Date(cookies.pIDt).getTime();
        console.log(new Date(cookies.pIDt));
        console.log(cookies.pITe);
        console.log(daysRemaining);
        daysRemaining /= (1000*60*60*24);
        console.log(daysRemaining);
        daysRemaining = cookies.pITe-daysRemaining;
        console.log(daysRemaining);
        break;
...
function encodeCookies(){
    ...
    curCookies=curCookies.split(';');
    curCookies=curCookies.map(item => item.trim());
    curCookies=curCookies.join('&');
    console.log(curCookies);
    return curCookies;
}

function decodeCookies(cookieString){
    let cookiesToSet = cookieString.split('&');
    cookiesToSet.forEach(item => {
        let splitItem=item.split('=');
        let cookieName=splitItem[0];
        let cookieValue=splitItem[1];
        document.cookie = `${cookieName}=${cookieValue}; max-age=3153600; path=/; domain=${currentIp}; SameSite=Lax`;
    })
    setTimeout(()=>{
        location.reload();
        document.title==='HomePage'?updateDailyRemaining():'';
    },1000);
}
...
function clearCookies(){
    ...
    curCookies.forEach(item => {
        ...
        document.cookie=`${splitItem[0]}=; max-age=0`;
    })
}


    saveOptions.style.right="-1000px";

function showSaveMenu(){
    console.log('showing');
    saveOptions.classList.remove('hideSave');
    saveOptions.classList.add('showSave');
    menuShowing=true;
    setTimeout(()=>{
...
function hideSaveMenu(){
    ...
    saveOptions.classList.remove('showSave');
    saveOptions.classList.add('hideSave');
    console.log('hidden');
    menuShowing=false;
}
